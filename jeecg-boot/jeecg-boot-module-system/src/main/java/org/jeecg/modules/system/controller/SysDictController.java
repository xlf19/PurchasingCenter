...
public class SysDictController {
	...
	/**
	 * 大数据量的字典表 走异步加载  即前端输入内容过滤数据distinct
	 * @param dictCode
	 * @return
	 */
	@RequestMapping(value = "/loadDictDistinct/{dictCode}", method = RequestMethod.GET)
	public Result<List<DictModel>> loadDictDistinct(@PathVariable String dictCode,@RequestParam(name="keyword") String keyword, @RequestParam(value = "sign",required = false) String sign,HttpServletRequest request) {
		log.info(" 加载字典表数据,加载关键字: "+ keyword);
		Result<List<DictModel>> result = new Result<List<DictModel>>();
		List<DictModel> ls = null;
		try {
			if(dictCode.indexOf(",")!=-1) {
				...
				ls = sysDictService.queryTableDictItems2(params[0],params[1],params[2],keyword);
				result.setSuccess(true);
				result.setResult(ls);
				log.info(result.toString());
			}else {

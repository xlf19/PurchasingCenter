...
export default {
  ...
  data() {
    return {
      ...
      contentMark: {},
      url: {
        list: '/contractedit/contractedit/templateid',
  ...
  methods: {
    ...
    addlist() {
      ...
      if (termsNumber > 0) {
        let cid = this.cid
        deleteAction(this.url.delete, { contract_id: cid }).then(res => {
          if (res.success) {
            this.addterms(cid)
          }
          if (res.code === 510) {
    ...
    templateDetail() {
      let cid = this.cid
      getAction(this.url.list, { template_id: this.templateId, cid: cid }).then(res => {
        let arr = res.result
